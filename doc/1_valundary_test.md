...
int型であるが 1 ~ 31の数値以外が渡されることはないとのこと。
無効同値クラスは考えない
...
| 29  | 20      | 戻り値20の代表値   |
| 30  | 20      | 戻り値20の上限代表値 |
| 31  | 0      | 戻り値0の下限境界値(上限境界値・代表値) |

## 1_11.ソースコードからテストケースを想定

今回も1~31以外の数値が渡されることはないとのこと。
今回はif文で1,2...30と日付が列挙されているため、以上・以下という考えた方ではない


| 値   | 期待される結果 | 備考                    |
| --- | ------- | --------------------- |
| 1   | 20      | 1による戻り値20の代表値(これしかない) |
| 2   | 20      |                       |
| 3   | 20      |                       |
| 4   | 20      |                       |
| 5   | 20      |                       |
